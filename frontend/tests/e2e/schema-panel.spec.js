...
test.describe("Schema Panel Component - Task 12 Validation", () => {
  ...
  test.describe("Component Rendering Tests", () => {
    test("should render dual panel layout", async ({ page }) => {
      const sourcePanelExists = await page
        .locator('[aria-label="Source Schema"]')
        .isVisible();
      const targetPanelExists = await page
        .locator('[aria-label="Target Schema"]')
        .isVisible();
    ...
    test("should display schema trees", async ({ page }) => {
      const sourceTrees = await page
        .locator('[aria-label="Source Schema"] [role="tree"]')
        .count();
      const targetTrees = await page
        .locator('[aria-label="Target Schema"] [role="tree"]')
        .count();
  ...
  test.describe("Field Type Icons", () => {
    test("should display correct icons for data types", async ({ page }) => {
      ...
      const integerIcon = await page
        .locator('[data-field-type="integer"] .field-icon')
        .first();
      await expect(integerIcon).toContainText("🔢");

      // Check varchar field icon
      const varcharIcon = await page
        .locator('[data-field-type="varchar"] .field-icon')
        .first();
      await expect(varcharIcon).toContainText("📝");

      // Check timestamp field icon
      const timestampIcon = await page
        .locator('[data-field-type="timestamp"] .field-icon')
        .first();
    ...
    test("should apply correct colors to field types", async ({ page }) => {
      ...
      const integerField = await page
        .locator('[data-field-type="integer"]')
        .first();
      const varcharField = await page
        .locator('[data-field-type="varchar"]')
        .first();
  ...
  test.describe("Search Functionality", () => {
    ...
    test("should filter target schema fields", async ({ page }) => {
      ...
      await page
        .locator('[aria-label="Target Schema"] [role="treeitem"]')
        .filter({ hasText: "dim_customers" })
        .locator(".expand-icon")
        .first()
        .click();
      await page.waitForTimeout(300);
      ...
      const customerKey = await page
        .locator('[role="treeitem"]')
        .filter({ hasText: "customer_key" });
  ...
  test.describe("Drag and Drop Functionality", () => {
    ...
    test("should perform drag and drop operation", async ({ page }) => {
      ...
      const dragEvent = await page
        .locator(".log-entry")
        .filter({ hasText: "drag-start" });
      const dropEvent = await page
        .locator(".log-entry")
        .filter({ hasText: "drop" });
  ...
  test.describe("Refresh Functionality", () => {
    test("should refresh source schema", async ({ page }) => {
      ...
      // Click refresh
      await page
        .locator('[aria-label="Source Schema"] button:text("Refresh")')
        .click();
      await page.waitForTimeout(300);
    ...
    test("should refresh target schema", async ({ page }) => {
      ...
      await page
        .locator('[aria-label="Target Schema"] button:text("Refresh")')
        .click();
      await page.waitForTimeout(300);
  ...
  test.describe("Edge Cases", () => {
    test("should handle empty search gracefully", async ({ page }) => {
      ...
      const treeItems = await page.locator(
        '[aria-label="Source Schema"] [role="treeitem"]',
      );
      expect(await treeItems.count()).toBeGreaterThan(0);
    });
    ...
    test("should handle non-matching search", async ({ page }) => {
      ...
      const visibleItems = await page.locator(
        '[aria-label="Source Schema"] [role="treeitem"]:visible',
      );
